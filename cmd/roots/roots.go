...
var HashTreeRootCmd, SigningRootCmd *cobra.Command
...
func MakeSigningRootCmd(st *spec_types.SpecType) *cobra.Command {
	return &cobra.Command{
		Use:   fmt.Sprintf("%s [input path]", st.Name),
		Short: fmt.Sprintf("Signing-root a %s, if the input path is not specified, input is read from STDIN", st.TypeName),
		Args:  cobra.ExactArgs(1),
		Run: func(cmd *cobra.Command, args []string) {
			var path string
			if len(args) > 0 {
			...
			root := ssz.SigningRoot(dst, st.SSZTyp)
			Report(cmd.OutOrStdout(), "%x\n", root)
		},
	}
}
...
func init() {
	...
	SigningRootCmd = &cobra.Command{
		Use:   "signing-root",
		Aliases: []string{"signing_root", "sigr"},
		Short: "signing-root SSZ data",
	}

	for _, st := range spec_types.SpecTypes {
		SigningRootCmd.AddCommand(MakeSigningRootCmd(st))
	}
}
